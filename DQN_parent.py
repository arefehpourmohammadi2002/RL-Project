...
class DQN(ABC):
    def __init__(self, input_dim, hidden_dim, output_dim,
                lr, target_update_counter, explore_update_counter,
                discount, epsilon, batch_size, RB_capacity, num_first_samples, num_epoches,
    ...
    def new_episode(self):
        ...
        mdp = MDP(num_nodes=self.num_nodes, depot_num=self.depot_num,
                 num_cars=self.num_cars, cars_capacity=self.cars_capacity)
    ...
    def create_target_input(self, rb_obj):
        ...
        for route in routes:
            ...
            for candid in candidates:
                target_model_input.append(self.create_input(mdp=mdp, routes=routes, route=route,
                                            next_node=candid, used=used, train=False))
    ...
    def target_result(self, rb_objs):
        answer_vec = []
        for rb_obj in rb_objs:
            target_input = self.create_target_input(rb_obj)
            if target_input is None:
                answer_vec.append(torch.tensor(0.0, device=self.device))
            else:
                with torch.no_grad():
                    answer_vec.append(self.target_model(target_input).max())
    ...
    def policy(self, train, candidate_map):
        ...
        for route_idx, candidates in candidate_map.items():
            ...
            for candidate in candidates:
                input.append(self.create_input(mdp=self.mdp, routes=self.routes, route=route,
                                                next_node=candidate, used=self.used, train=False))
    ...
    def add_route(self, route_idx, next_node):
    ...
    def run_episode(self, train=True):
        ...
        while candidate_map:
            ...
            self.add_route(route_idx, next_node)
            candidate_map = self.collect_candidates()
    ...
    def evaluate(self, mdp):
        self.mdp = mdp
        self.routes = [self.new_route() for _ in range(self.num_cars)]
