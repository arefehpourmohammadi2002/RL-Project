...
class MDP():
    ...
    def fill_distance_matrix(self, min_distance, max_distance):
        ...
        node_dis_sum = sum(self.distance_matrix[:, i] for i in range(len(self.num_nodes))) 
        self.distance_matrix_ave = (node_dis_sum[i] / (len(self.num_nodes)-1)
                            for i in range(self.num_nodes))
        self.distance_matrix_std = statistics.pstdev(self.node_demand[i] 
                                              for i in range(self.num_nodes) 
                                              if i != self.depot_num)
    ...
    def fill_node_dem_matrix(self, min_node_dem, max_node_dem):
        ...
        node_dem_sum = sum(self.node_capacity[i] for i in range(len(self.node_capacity))) 
        self.node_dem_ave = node_dem_sum / (len(self.node_capacity)-1)
        self.node_dem_std = statistics.pstdev(self.node_demand[i] 
                                              for i in range(self.num_nodes) 
                                              if i != self.depot_num)
